...
public class UserCamera implements SurfaceHolder.Callback{
	...
	public void surfaceChanged(SurfaceHolder holder, int format, int width,
		...
		int bestWidth=0;
		int bestHeight=0;
		int LARGEST_HEIGHT=600;
		int LARGEST_WIDTH=800;
		Camera.Parameters parameters = sCamera.getParameters(); 
		List<Camera.Size> picturesSizes=parameters.getSupportedPictureSizes();
		List<Camera.Size> previewSizes = parameters.getSupportedPreviewSizes();
		Size lSize=picturesSizes.get(1);
		if (picturesSizes.size()>1){
			
			Iterator<Camera.Size> sps=picturesSizes.iterator();
			while(sps.hasNext()){
				Camera.Size sSize=sps.next();
				Log.v("picture","checking"+sSize.width+"x"+sSize.height);
				if (sSize.width==2048&&sSize.height==1152){
					lSize=sSize;
					break;
				}
				else if(sSize.width<2048){
					lSize=sSize;
					break;
				}
			}
		...
		Log.v("using","using"+lSize.width+"x"+lSize.height);
        ...
        parameters.setPictureSize(lSize.width, lSize.height); // 设置保存的图片尺寸
